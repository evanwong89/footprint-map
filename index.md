...
- 状态：MVP 与 v0.28 已实现，并通过类型检查、47 项自动化测试、生产构建及本地浏览器滚轮缩放、拖动和关闭复位验证；待 Obsidian Mobile 物理触屏捏合与真实高德 Key 验收。
- 关联需求：[[../../Requirements/REQ-004-可移植Markdown互动足迹地图|REQ-004：可移植 Markdown 互动足迹地图]]
- 当前范围：Obsidian 日记内嵌互动地图、在命令触发时的光标处插入代码块、Leaflet/OpenStreetMap 与高德静态地图底图、照片缩略图点位、地图外横向照片浏览、单击大图预览及 `1×–6×` 缩放拖动、200 米连续照片聚合、Vault 内 JPEG/HEIC/PNG 的 TypeScript EXIF 解析、GeoJSON 通用数据和独立 HTML 查看器。
