...
- 状态：v0.28 实施基线。
- TRD 版本：v0.5。
- 需求基线：[[../../Requirements/REQ-004-可移植Markdown互动足迹地图|REQ-004 v0.28]]。
- 项目设计：[[design|设计图]]、[[interaction|交互图]]、[[visual|效果图]]。
- 实施状态：MVP 与 v0.28 源码、Obsidian 包与 standalone 产物已创建，并通过类型检查、47 项自动化测试、生产构建及本地浏览器滚轮缩放、拖动和关闭复位验证；待 Obsidian Mobile 物理触屏捏合与真实高德 Key 验收。
...
- 地图外照片浏览区承接点位选择状态，横向滚动展示该点全部照片，单张图片高度不超过 4 cm；地图画布上不打开点位照片弹窗。用户单击浏览区照片时，由共享渲染层打开居中大图浮框；滚轮与触控板使用非被动 wheel 事件围绕指针缩放，Pointer Events 处理双指捏合与放大后拖动，纯计算模块把比例限制在 `1×–6×` 并约束平移边界。浮框继续支持关闭按钮、背景、`Esc` 与键盘确认操作。
- 高德 Web 服务请求保持整数缩放，Leaflet 视野改用 `0.25` 级细分。静态图请求优先使用能覆盖视口的较清晰整数源级别，并根据返回图像像素、中心点和源级别计算独立地理边界；视野适配按固定尺寸照片卡、序号徽标、全部坐标和当前容器尺寸计算可容纳的最高细分级别，消除整级回退造成的明显留白。
...
- 不在源码中固定私有 API Key。
- 自动运行时网络请求只用于用户启用的地图底图；EXIF、GeoJSON、照片和静态降级全部在本地。
- 设置页与 Obsidian 地图面板标题区右上方提供公开 Tally 表单入口。面板入口位于“适配全部点位”旁并保持次级视觉层级。只有用户点击后，才由当前窗口打开独立浏览器页面；插件不读取、预填或上传 Vault 内容、照片、坐标、GeoJSON、设置或诊断信息。
- 共享地图渲染器通过可选 `onFeedback` 回调接收平台操作；Obsidian 适配层注入该回调，standalone 默认不注入，避免通用核心硬编码第三方产品入口。
...
- 在 README 和设置页披露可选反馈入口由第三方 Tally 提供，仅在用户主动点击后访问，并提醒用户不要提交敏感足迹材料。
